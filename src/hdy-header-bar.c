...
G_DEFINE_TYPE_WITH_CODE (HdyHeaderBar, hdy_header_bar, GTK_TYPE_WIDGET,
...
static void
update_title (HdyHeaderBar *self)
{
  GtkRoot *root;
...
static void
construct_title_label (HdyHeaderBar *self)
{
  GtkWidget *label;
...
static void
hdy_header_bar_root (GtkWidget *widget)
{
  GtkWidget *root;

  GTK_WIDGET_CLASS (hdy_header_bar_parent_class)->root (widget);
  ...
  update_title (HDY_HEADER_BAR (widget));
}

static void
hdy_header_bar_unroot (GtkWidget *widget)
{
  g_signal_handlers_disconnect_by_func (gtk_widget_get_root (widget),
                                        update_title, widget);

  GTK_WIDGET_CLASS (hdy_header_bar_parent_class)->unroot (widget);
}
...
hdy_header_bar_get_request_mode (GtkWidget *widget)
{
  GtkWidget *w;
  int wfh = 0, hfw = 0;
...
hdy_header_bar_class_init (HdyHeaderBarClass *class)
{
  ...
  widget_class->root = hdy_header_bar_root;
  widget_class->unroot = hdy_header_bar_unroot;
  widget_class->get_request_mode = hdy_header_bar_get_request_mode;
                                    ...
                                    const gchar  *type)
{
  if (g_strcmp0 (type, "title") == 0)
    hdy_header_bar_set_title_widget (HDY_HEADER_BAR (buildable), GTK_WIDGET (child));
  else if (g_strcmp0 (type, "start") == 0)
    hdy_header_bar_pack_start (HDY_HEADER_BAR (buildable), GTK_WIDGET (child));
  else if (g_strcmp0 (type, "end") == 0)
    hdy_header_bar_pack_end (HDY_HEADER_BAR (buildable), GTK_WIDGET (child));
...
/**
 * hdy_header_bar_set_title_widget:
 * @self: a #HdyHeaderBar
 * @title_widget: (allow-none): a widget to use for a title
 *
 * Sets the title for the #HdyHeaderBar.
 *
 ...
 */
void
hdy_header_bar_set_title_widget (HdyHeaderBar *self,
                                 GtkWidget    *title_widget)
{
...
/**
 * hdy_header_bar_get_title_widget:
 * @self: a #HdyHeaderBar
 *
 ...
 */
GtkWidget *
hdy_header_bar_get_title_widget (HdyHeaderBar *self)
{
...
/**
 * hdy_header_bar_remove:
 * @self: a #HdyHeaderBar
 * @child: the child to remove
 *
 * Removes a child from @self, after it has been added
 * with hdy_header_bar_pack_start(), hdy_header_bar_pack_end()
 * or hdy_header_bar_set_title_widget().
 */
void
hdy_header_bar_remove (HdyHeaderBar *self,
                       GtkWidget    *child)
{
  GtkWidget *parent;
                                     ...
                                     HdyCenteringPolicy  centering_policy)
{
  ...
  if (self->centering_policy == HDY_CENTERING_POLICY_STRICT) {
